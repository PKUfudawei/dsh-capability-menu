...
| 在长列表里找能力 | 页签栏下方的过滤框，按名字过滤当前 tab（Tools / Skills 共用） |
...
**页面结构**：Tools 页按 server 分组、可折叠，MCP 工具挂在各自 server 下，内置原生工具统一在「系统内置」组；Skills 页分「全局技能 / 项目技能 / 预设技能」子页签——**「预设技能」只在确实存在随 agent preset 分发的技能时出现**（没有就是原来的两个页签），组内以 preset id 作小标题，不再往下分层。页签下方是按名字过滤当前 tab 的过滤框（列表长时比翻分组快）。点能力行看模型侧的工具定义，点技能行展开目录树、点文件预览正文。
