    ...
    const tool = defineTool({
        ...
        parameters: {
            ...
            args: {
                ...
                description: 'Arguments for the target tool, passed directly. Each property must be a valid value for the target tool schema (the parameter object from meta_search detail). ' +
...
const ENVELOPE_KEYS = ['id', 'kind', 'args'];
/**
 ...
 */
function envelopeDiagnostic(forwarded, targetName, parameters) {
    const keys = Object.keys(forwarded);
    if (keys.length !== ENVELOPE_KEYS.length)
        return undefined;
    if (!ENVELOPE_KEYS.every(key => keys.includes(key)))
        return undefined;
    if (forwarded['kind'] !== 'tool' || forwarded['id'] !== targetName)
        return undefined;
    const payload = forwarded['args'];
    if (payload === null || typeof payload !== 'object' || Array.isArray(payload))
        return undefined;
    const declared = parameters?.properties;
    if (declared !== undefined && (Object.hasOwn(declared, 'id') || Object.hasOwn(declared, 'kind')))
        return undefined;
    return (`meta_invoke: args looks like a nested { id, kind, args } envelope for "${targetName}" — ` +
