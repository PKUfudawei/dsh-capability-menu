...
export function defaultPatchFile() {
...
function* entryNodes(doc) {
    ...
    for (const item of root.items) {
        ...
        if (insert !== undefined && insert !== null) {
            if (!YAML.isSeq(insert))
                continue;
            for (const row of insert.items) {
                if (YAML.isMap(row))
                    yield { node: row, parent: insert };
            }
            continue;
        }
        if (item.get('id') !== undefined)
            yield { node: item, parent: root };
    }
}
/** Find the entry row with `id`, if the file declares one. */
export function findEntry(doc, id) {
...
export function addEntry(doc, entry) {
    const root = doc.contents;
    if (!YAML.isSeq(root))
        throw new Error('patch file root is not a sequence');
    const target = lastInsertSeq(doc) ?? (() => {
        const block = doc.createNode({ insert: [] });
 ...
 */
export function addEntryOverride(doc, entry) {
    const root = doc.contents;
    if (!YAML.isSeq(root))
        throw new Error('patch file root is not a sequence');
    // `root` is narrowed to a parsed sequence; the freshly built map is not
    // narrowed with it even though it is a real document node.
    root.add(doc.createNode({ ...entry }));
}
...
export function setEntryDisabled(doc, id, disabled) {
...
export function setEntryConfig(doc, id, config) {
    const node = findEntry(doc, id);
    if (node === undefined)
        return false;
 ...
 */
export async function mutatePatch(file, mutate) {
