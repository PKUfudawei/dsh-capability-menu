...
import { addEntry, defaultPatchFile, mutatePatch, readEntries, removeEntry, setEntryConfig, setEntryDisabled, } from "./patch-file.js";
...
export class LocationRegistry {
    ...
    async listSkills() {
        ...
        for (const name of names) {
            ...
            if (statTarget?.isDirectory() !== true)
                continue;
    ...
    async addSkill(dir) {
        ...
        if (!(await hasSkillManifest(target)))
            throw new Error(`目录中没有 SKILL.md：${dir}`);
